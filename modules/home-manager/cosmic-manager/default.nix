{ config, lib, pkgs, inputs, cosmicLib, ... }:

{
  options = {
    cosmic-manager.enable =
      lib.mkEnableOption "enables cosmic-manager customisations";
  };
  ...
  config = lib.mkIf config.cosmic-manager.enable {
    ...
    wayland.desktopManager.cosmic.panels = [{
      ...
      plugins_center =
        cosmicLib.cosmic.mkRON "optional" [ "com.system76.CosmicAppletTime" ];
      plugins_wings = cosmicLib.cosmic.mkRON "optional"
        (cosmicLib.cosmic.mkRON "tuple" [
          [ "com.system76.CosmicAppletWorkspaces" "com.system76.CosmicAppList" ]
          [
    ...
    home.packages = with pkgs; [ inter jetbrains-mono ];
