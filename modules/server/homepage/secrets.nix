            ...
            stringData = {
              ...
              GRAFANA_PASSWORD =
                if (config.monitoring.grafana.enable && config.secrets.grafana.enable) then
                  ...
                  "";
              NEXTCLOUD_USERNAME =
                if (config.nextcloud.enable && config.secrets.nextcloud.enable) then
              ...
              NEXTCLOUD_PASSWORD =
                if (config.nextcloud.enable && config.secrets.nextcloud.enable) then
                  ...
                  "";
              FORGEJO_KEY =
                if (config.forgejo.enable && config.secrets.forgejo.enable) then
                  config.sops.placeholder."forgejo/key"
                else
                  "";
              IMMICH_KEY =
                if (config.immich.enable && config.secrets.immich.enable) then
